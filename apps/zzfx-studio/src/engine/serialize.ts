...
// Export a usable ESM file — ready to drop into a game project
...
export function songToCode(song: Song, { includeMetadata = true }: { includeMetadata?: boolean } = {}): string {
  const expanded = songToZzfxm(song);

  const lines: string[] = [];

  lines.push('// Generated by ZzFX Studio — https://github.com/thejustinwalsh/zzfx-studio');
  if (includeMetadata) {
  ...
  if (includeMetadata) {
    lines.push("import { zzfxm } from '@zzfx-studio/zzfxm/micro';");
  } else {
    lines.push("import { ZZFX } from 'zzfx';");
    lines.push("import { zzfxm } from '@zzfx-studio/zzfxm';");
  }
  lines.push('');
  ...
  lines.push('zzfxm(instruments, patterns, sequence, BPM);');

  return lines.join('\n');
}

// Compact snippet for clipboard (expanded for correct playback)
