...
test('every drum sound the generator emits renders cleanly', () => {
  ...
  // Annotated rather than `as const`: a const assertion cannot be applied to a
  // conditional expression, only to a literal.
  const voiceOf = (n: number): 'KICK' | 'SNARE' | 'HAT' =>
    n <= 6 ? 'KICK' : n <= 22 ? 'SNARE' : 'HAT';
