...
// Higher = snappier onset, lower = longer trails
...
export function Oscilloscope({
  ...
  // Mutable current RGB values for lerping — avoids allocations per frame
  ...
  const tick = useCallback(() => {
    ...
    for (let i = 0; i < barCount; i++) {
      ...
      const cur = current[i];
      // Use fast attack for new colors, slow decay toward grey
  ...
  }, [isPlaying, analyser, tick, barCount]);
        ...
        return (
          <View
            ...
            style={[
              ...
              {
                height: barHeight,
                backgroundColor: val > 0.01 ? displayColors[i] : colors.bgElevated,
                opacity: val > 0.01 ? 0.5 + val * 0.5 : 0.15,
              },
            ]}
          />
        );
